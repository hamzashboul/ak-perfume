...
export default function CartDrawer() {
  ...
  const waMsg = lang === 'ar'
    ...
    : 'Hello, I want to order:\n' + items.map(i => `- ${i.nameEn} × ${i.quantity} = ${(i.price * i.quantity).toFixed(2)} JD`).join('\n') + `\nTotal: ${totalPrice().toFixed(2)} JD`;
      ...
      <div dir={isRTL ? 'rtl' : 'ltr'} style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 'min(420px, 100vw)', zIndex: 61, background: '#0A0A0A', borderLeft: '0.5px solid rgba(201,169,110,0.12)', display: 'flex', flexDirection: 'column', transform: isOpen ? 'translateX(0)' : 'translateX(100%)', transition: 'transform 400ms cubic-bezier(0.16,1,0.3,1)', boxShadow: isOpen ? '-24px 0 64px rgba(10,10,10,0.5)' : 'none' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 24px', borderBottom: '0.5px solid rgba(201,169,110,0.1)', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1.375rem', fontWeight: 400, color: '#F8F6F2' }}>{t.cart.title[lang]}</div>
            {totalItems() > 0 && (
              ...
              {items.map((item, i) => (
                <div key={item.id} style={{ display: 'flex', gap: '14px', padding: '16px 0', borderBottom: i < items.length - 1 ? '0.5px solid rgba(201,169,110,0.08)' : 'none' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1.0625rem', fontWeight: 400, color: '#F8F6F2', marginBottom: '3px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
        ...
        {items.length > 0 && (
          <div style={{ padding: '20px 24px', borderTop: '0.5px solid rgba(201,169,110,0.1)', flexShrink: 0, background: '#0A0A0A' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '16px' }}>
              <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.6875rem', fontWeight: 500, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(248,246,242,0.45)' }}>{t.cart.total[lang]}</span>
