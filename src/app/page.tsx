  ...
  return (
    <div dir={isRTL ? 'rtl' : 'ltr'} style={{ background: '#F8F6F2' }}>
      ...
      <section style={{ minHeight: '100vh', background: '#0A0A0A', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden', marginTop: 'calc(var(--nav-h) * -1)', paddingTop: 'var(--nav-h)', textAlign: 'center' }}>
        ...
        <motion.div animate={{ opacity: [0.4, 0.9, 0.4], scale: [1, 1.15, 1] }} transition={{ duration: 5, repeat: Infinity, ease: 'easeInOut' }} style={{ position: 'absolute', inset: 0, pointerEvents: 'none', background: 'radial-gradient(ellipse 70% 50% at 50% 30%, rgba(201,169,110,0.08) 0%, transparent 65%)' }}/>
        <motion.div animate={{ opacity: [0.2, 0.5, 0.2], scale: [1.1, 1, 1.1] }} transition={{ duration: 7, repeat: Infinity, ease: 'easeInOut', delay: 1 }} style={{ position: 'absolute', inset: 0, pointerEvents: 'none', background: 'radial-gradient(ellipse 50% 40% at 30% 60%, rgba(201,169,110,0.05) 0%, transparent 70%)' }}/>
        ...
        <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', pointerEvents: 'none' }} xmlns="http://www.w3.org/2000/svg">
          {decorLines.map((line, i) => (
            <motion.line key={i} x1={line.x1} y1={line.y1} x2={line.x2} y2={line.y2} stroke="#C9A96E" strokeWidth="0.5" strokeLinecap="round" initial={{ opacity: 0 }} animate={{ opacity: [0, 0.35, 0.1, 0.35, 0], y: [0, -15, 0] }} transition={{ duration: 4 + i * 0.5, repeat: Infinity, delay: line.delay, ease: 'easeInOut' }}/>
          ))}
        </svg>

        {[{ top: '12%', left: '12%' }, { top: '12%', right: '12%' }, { bottom: '18%', left: '18%' }, { bottom: '18%', right: '18%' }].map((pos, i) => (
          <motion.div key={i} animate={{ opacity: [0.1, 0.4, 0.1], scale: [0.8, 1.2, 0.8] }} transition={{ duration: 3 + i * 0.6, repeat: Infinity, delay: i * 0.5, ease: 'easeInOut' }} style={{ position: 'absolute', width: '4px', height: '4px', background: '#C9A96E', transform: 'rotate(45deg)', pointerEvents: 'none', ...pos }}/>
        ))}

        <div style={{ position: 'relative', zIndex: 2, padding: '3rem var(--site-px) 6rem' }}>

          <motion.div initial={{ opacity: 0, y: 20 }} animate={loaded ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.9, delay: 0.1, ease: [0.16, 1, 0.3, 1] as [number,number,number,number] }} style={{ marginBottom: '2.5rem' }}>
            <div style={{ position: 'relative', display: 'inline-block' }}>
              <motion.div animate={{ opacity: [0, 0.15, 0], scale: [0.8, 1.3, 0.8] }} transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }} style={{ position: 'absolute', inset: '-20px', background: 'radial-gradient(ellipse 80% 60% at 50% 50%, rgba(201,169,110,0.4) 0%, transparent 70%)', borderRadius: '50%', pointerEvents: 'none', zIndex: 0 }}/>
              <motion.div animate={{ textShadow: ['0 0 0px rgba(201,169,110,0)', '0 0 30px rgba(201,169,110,0.25), 0 0 60px rgba(201,169,110,0.1)', '0 0 0px rgba(201,169,110,0)'] }} transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }} style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(2.5rem,7vw,5rem)', fontWeight: 300, letterSpacing: '0.12em', color: '#F8F6F2', lineHeight: 1, marginBottom: '10px', position: 'relative', zIndex: 1 }}>
                AK PERFUMES
              </motion.div>
            </div>
            <motion.div initial={{ scaleX: 0 }} animate={loaded ? { scaleX: 1 } : {}} transition={{ duration: 0.9, delay: 0.6 }} style={{ width: '100%', height: '0.5px', background: 'linear-gradient(90deg, transparent, rgba(201,169,110,0.6), transparent)', marginBottom: '10px', transformOrigin: 'center' }}/>
            <motion.div animate={{ opacity: [0.6, 1, 0.6] }} transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }} style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.6875rem', fontWeight: 500, letterSpacing: '0.32em', textTransform: 'uppercase', color: '#C9A96E' }}>
              {t.common.defineYourself[lang]}
            </motion.div>
          </motion.div>

          <motion.div initial={{ opacity: 0 }} animate={loaded ? { opacity: 1 } : {}} transition={{ duration: 0.6, delay: 0.4 }} style={{ display: 'flex', alignItems: 'center', gap: '12px', maxWidth: '200px', margin: '0 auto 2.5rem' }}>
            <motion.div animate={{ scaleX: [0.3, 1, 0.3], opacity: [0.3, 0.8, 0.3] }} transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }} style={{ flex: 1, height: '0.5px', background: 'rgba(201,169,110,0.5)', transformOrigin: 'right' }}/>
            <motion.div animate={{ rotate: [45, 90, 45], scale: [1, 1.3, 1] }} transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }} style={{ width: '5px', height: '5px', background: '#C9A96E', flexShrink: 0 }}/>
            <motion.div animate={{ scaleX: [0.3, 1, 0.3], opacity: [0.3, 0.8, 0.3] }} transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut', delay: 0.5 }} style={{ flex: 1, height: '0.5px', background: 'rgba(201,169,110,0.5)', transformOrigin: 'left' }}/>
          </motion.div>

          <motion.h1 initial={{ opacity: 0, y: 30 }} animate={loaded ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8, delay: 0.45, ease: [0.16, 1, 0.3, 1] as [number,number,number,number] }} style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(2rem,5vw,3.5rem)', fontWeight: 300, lineHeight: 1.1, letterSpacing: '-0.03em', color: '#F8F6F2', marginBottom: '1rem' }}>
            {t.home.heroTitle[lang]}{' '}
            <em style={{ fontStyle: 'italic', color: '#C9A96E' }}>{t.home.heroAccent[lang]}</em>
          </motion.h1>

          <motion.p initial={{ opacity: 0 }} animate={loaded ? { opacity: 1 } : {}} transition={{ duration: 0.7, delay: 0.6 }} style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.875rem', fontWeight: 300, color: 'rgba(248,246,242,0.4)', lineHeight: 1.8, maxWidth: '500px', margin: '0 auto 2rem' }}>
            {t.home.heroSub[lang]}
          </motion.p>

          <motion.div initial={{ opacity: 0, y: 20 }} animate={loaded ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: 0.75, ease: [0.16, 1, 0.3, 1] as [number,number,number,number] }} style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: '3rem', justifyContent: 'center', marginTop: '3.5rem' }}>
            {[
              ...
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={loaded ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 1 + i * 0.15 }} style={{ textAlign: 'center' }}>
        ...
        <motion.div initial={{ opacity: 0 }} animate={loaded ? { opacity: 0.45 } : {}} transition={{ duration: 0.7, delay: 1.6 }} style={{ position: 'absolute', bottom: '2rem', left: '50%', transform: 'translateX(-50%)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', zIndex: 3, pointerEvents: 'none' }}>
          <motion.div animate={{ height: ['36px', '18px', '36px'], opacity: [1, 0.3, 1] }} transition={{ duration: 1.5, repeat: Infinity, ease: 'easeInOut' }} style={{ width: '0.5px', background: 'linear-gradient(180deg, #C9A96E, transparent)' }}/>
      ...
      <div style={{ background: '#0A0A0A', borderBottom: '0.5px solid rgba(201,169,110,0.1)', padding: '11px 0', overflow: 'hidden' }}>
        <motion.div animate={{ x: ['0%', '-50%'] }} transition={{ duration: 24, repeat: Infinity, ease: 'linear' }} style={{ display: 'flex', whiteSpace: 'nowrap' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '5rem', alignItems: 'center' }}>
            ...
            <Reveal variants={slideRight} delay={0.2}>
              <div style={{ display: 'flex', justifyContent: 'center' }}>
                <motion.div whileHover={{ y: -10, boxShadow: '0 36px 90px rgba(10,10,10,0.2)' }} transition={{ duration: 0.4 }} style={{ width: '220px', height: '300px', background: '#0A0A0A', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', boxShadow: '0 24px 64px rgba(10,10,10,0.15)' }}>
                  <motion.div animate={{ opacity: [0.3, 0.7, 0.3] }} transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }} style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 70% 50% at 50% 30%, rgba(201,169,110,0.12) 0%, transparent 70%)', borderRadius: '4px' }}/>
                  <motion.svg width="80" height="150" viewBox="0 0 80 150" fill="none" animate={{ y: [0, -8, 0] }} transition={{ duration: 5, repeat: Infinity, ease: 'easeInOut' }}>
        ...
        <div className="site-container" ref={whyRef}>
          ...
          <motion.div initial="hidden" animate={whyInView ? 'visible' : 'hidden'} variants={stagger} style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1px', background: 'rgba(10,10,10,0.06)' }}>
            {[
              ...
              <motion.div key={i} variants={fadeUp} custom={i * 0.1} whileHover={{ background: '#FAFAF8' }} style={{ background: '#FFFFFF', padding: 'clamp(2rem,4vw,3rem)', textAlign: 'center' }}>
                <motion.div animate={{ scale: [1, 1.2, 1] }} transition={{ duration: 3, repeat: Infinity, delay: i * 0.8, ease: 'easeInOut' }} style={{ fontSize: '1.125rem', color: '#C9A96E', marginBottom: '1.25rem', opacity: 0.7 }}>
                  {f.icon}
                </motion.div>
      ...
      <section style={{ background: '#0A0A0A', paddingBlock: 'clamp(4rem,8vw,6rem)', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        <motion.div animate={{ scale: [1, 1.15, 1], opacity: [0.4, 0.8, 0.4] }} transition={{ duration: 5, repeat: Infinity, ease: 'easeInOut' }} style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 60% 60% at 50% 50%, rgba(201,169,110,0.06) 0%, transparent 70%)', pointerEvents: 'none' }}/>
        <div className="site-container" style={{ position: 'relative', zIndex: 1 }}>
          <Reveal variants={fadeUp}>
            ...
            <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(2.5rem,5vw,4rem)', fontWeight: 300, letterSpacing: '-0.03em', color: '#F8F6F2', marginBottom: '1rem' }}>{t.home.ctaTitle[lang]}</h2>
            <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.875rem', fontWeight: 300, color: 'rgba(248,246,242,0.35)', marginBottom: '2.5rem' }}>{t.home.ctaSub[lang]}</p>
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
              <motion.div whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}>
                <Link href="/products" style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.6875rem', fontWeight: 500, letterSpacing: '0.18em', textTransform: 'uppercase', background: '#C9A96E', color: '#0A0A0A', padding: '13px 28px', borderRadius: '1px', textDecoration: 'none', display: 'inline-block' }}>{t.home.shopNow[lang]}</Link>
              </motion.div>
              <motion.div whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}>
                <a href="https://wa.me/962787304077" target="_blank" rel="noopener noreferrer" style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.6875rem', fontWeight: 500, letterSpacing: '0.18em', textTransform: 'uppercase', background: 'transparent', color: '#F8F6F2', padding: '12px 28px', borderRadius: '1px', border: '0.5px solid rgba(248,246,242,0.18)', textDecoration: 'none', display: 'inline-block' }}>{t.home.whatsapp[lang]}</a>
              </motion.div>
            </div>
          </Reveal>
        </div>
      ...
      <footer style={{ background: '#050505', borderTop: '0.5px solid rgba(201,169,110,0.08)', padding: '2.5rem 0' }}>
        <div className="site-container" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          <motion.div whileHover={{ letterSpacing: '0.15em' }} transition={{ duration: 0.3 }} style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1.125rem', fontWeight: 400, letterSpacing: '0.1em', color: '#F8F6F2', display: 'flex', alignItems: 'center', gap: '6px' }}>
            AK <motion.span animate={{ rotate: 360 }} transition={{ duration: 8, repeat: Infinity, ease: 'linear' }} style={{ color: '#C9A96E', fontSize: '0.625rem', display: 'inline-block' }}>✦</motion.span> PERFUMES
          </motion.div>
          <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.5625rem', fontWeight: 400, letterSpacing: '0.12em', color: 'rgba(248,246,242,0.18)' }}>{t.home.footerCopy[lang]}</p>
        </div>
      </footer>
    </div>
  );
